...
if use_30_confirmation:
    ...
    candle_numbers_30 = st.multiselect("30-min Candle Numbers", options=list(range(2, 10)), default=[2])

    valid_dates_30, missing_30 = [], []
    for date in filtered['Date'].dt.date.unique():
        ...
        high, low = first_candle['high'], first_candle['low']

        df_window = df_day[df_day['candle_number'].isin(candle_numbers_30)]
        condition_met = False
        breakout_candle = None
        ...
        if condition_met:
            ...
            if breakout_candle is not None:
                ...
                breakout_moves_30[date] = round(last_close - breakout_close, 2)

    st.info(f"Out of {len(filtered)} days, {len(valid_dates_30)} matched 30-min confirmation")
    if missing_30:
        st.warning(f"{len(missing_30)} days skipped due to missing 30-min data")
    filtered = filtered[filtered['Date'].dt.date.isin(valid_dates_30)]
...
if len(filtered) > 0:
    ...
    if use_confirmation and breakout_moves:
        filtered['5_Move'] = filtered['Date'].dt.date.map(breakout_moves)

    if use_30_confirmation and breakout_moves_30:
        filtered['30_Move'] = filtered['Date'].dt.date.map(breakout_moves_30)

    filtered_display = filtered.copy()
    filtered_display['Date'] = pd.to_datetime(filtered_display['Date'])
    ...
    if '30_Move' in filtered.columns:
        display_cols.append('30_Move')
    ...
    if period_option == "Month":
        ...
        df['Period'] = df['Date'].dt.year.astype(int).astype(str)
